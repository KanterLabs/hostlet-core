...
pub(crate) async fn create_and_send_rollback(
    ...
    let target_services = sqlx::query(
        "SELECT service_name,role,container_name,image_tag,target_port,published_port,status,health_status \
         FROM deployment_services WHERE deployment_id=$1 ORDER BY created_at ASC",
    )
    ...
    .map(|row| json!({
        ...
        "imageTag": row.get::<Option<String>, _>("image_tag"),
        "targetPort": row.get::<Option<i32>, _>("target_port"),
        "publishedPort": row.get::<Option<i32>, _>("published_port"),
