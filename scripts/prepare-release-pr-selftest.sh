...
git -C "${WORK}" push --quiet origin \
  "HEAD:refs/heads/release-candidate/v9.9.9"
if (cd "${WORK}" && GH_BIN="${GH_STUB}" GITHUB_REPOSITORY=example/hostlet-core \
    bash "${SCRIPT}" --repo example/hostlet-core 9.9.9 >/dev/null 2>&1); then
...
assert value["head"] == "release-candidate/v0.3.0"
